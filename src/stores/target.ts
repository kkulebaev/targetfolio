...
export const useTargetStore = defineStore("target", () => {
  ...
  function setCustomWeights(weights: TargetWeight[]) {
    const seen = new Set<Ticker>();
    const valid: TargetWeight[] = [];
    for (const w of weights) {
      if (!INSTRUMENTS_BY_TICKER.has(w.ticker)) continue;
      if (seen.has(w.ticker)) continue;
      seen.add(w.ticker);
      valid.push({ ticker: w.ticker, weightPercent: w.weightPercent });
    }
    targetWeights.value = valid;
    currentPreset.value = "custom";
  }
